...
const AdminDashboard = () => {
  ...
  const handleVerification = (id, action) => {
    setPendingTherapists((prev) =>
      prev.map((therapist) => (therapist.id === id ? { ...therapist, status: action } : therapist)),
    )

    // Simulate sending email
    const therapist = pendingTherapists.find((t) => t.id === id)
    if (action === "approved") {
      alert(`Verification email sent to ${therapist.email}`)
    } else {
      alert(`Rejection email sent to ${therapist.email}`)
    }
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 sm:py-8">
        <div className="mb-6 sm:mb-8">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-2">Therapist Verification Requests</h2>
          <p className="text-gray-600 text-sm sm:text-base">Review and approve therapist applications</p>
        </div>
                ...
                {pendingTherapists.map((therapist) => (
                  <tr key={therapist.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 sm:px-6 py-4">
                      <div className="text-sm text-gray-900 max-w-xs truncate">{therapist.address}</div>
                    </td>
                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
